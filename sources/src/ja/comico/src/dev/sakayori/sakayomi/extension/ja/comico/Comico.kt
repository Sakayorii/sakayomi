package dev.sakayori.sakayomi.extension.ja.comico
...
class Comico : HttpSource() {
    ...
    override val client = network.cloudflareClient.newBuilder()
        .cookieJar(
            object : CookieJar {
                override fun saveFromResponse(url: HttpUrl, cookies: List<Cookie>) = cookies.filter { it.matches(url) }.forEach {
    ...
    private fun search(query: String, page: Int) = FormBody.Builder().add("query", query)
        .add("pageNo", (page - 1).toString())
        .add("pageSize", "25").build()

    private fun paginate(route: String, page: Int) = GET("$apiUrl/$route?pageNo=${page - 1}&pageSize=25", apiHeaders)
