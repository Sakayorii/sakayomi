...
class TriStateFilterOption(name: String, val value: String) : dev.sakayori.sakayomi.source.model.Filter.TriState(name)
abstract class TriStateGroupFilter(name: String, options: List<TriStateFilterOption>) : dev.sakayori.sakayomi.source.model.Filter.Group<TriStateFilterOption>(name, options) {
...
fun dev.sakayori.sakayomi.source.model.Filter<*>.toJsonObject(key: String): Filter {
...
open class TextFilter(name: String) : dev.sakayori.sakayomi.source.model.Filter.Text(name)

class GenreGroupFilter(filters: List<TriStateFilterOption>) : TriStateGroupFilter("Genres", filters)

class CheckboxFilterOption(name: String, val value: String, default: Boolean = true) : dev.sakayori.sakayomi.source.model.Filter.CheckBox(name, default)

abstract class CheckboxGroupFilter(name: String, options: List<CheckboxFilterOption>) : dev.sakayori.sakayomi.source.model.Filter.Group<CheckboxFilterOption>(name, options) {
...
abstract class SelectFilter(name: String, private val options: List<SelectFilterOption>, default: Int = 0) : dev.sakayori.sakayomi.source.model.Filter.Select<String>(name, options.map { it.name }.toTypedArray(), default) {
    val selected: String
...
fun getSortFilters(sortState: Int, lusLang: String) = FilterList(
    ...
    dev.sakayori.sakayomi.source.model.Filter.Header("Separate tags with commas (,)"),
    dev.sakayori.sakayomi.source.model.Filter.Header("Prepend with dash (-) to exclude"),
    TagTextFilters(),
    dev.sakayori.sakayomi.source.model.Filter.Header("The following require username or ID"),
    CreatorTextFilters(),
    FavoriteTextFilters(),
)
