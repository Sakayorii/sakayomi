package dev.sakayori.sakayomi.extension.zh.manhuagui
...
import Sakayorii.lib.lzstring.LZString
import Sakayorii.lib.unpacker.Unpacker
...
class Manhuagui(
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        if (hiddenEncryptedChapterList != null) {
            if (getShowR18()) {
                ...
                // "You need to enable R18 switch and restart Sakayomi to read this manga"
    ...
    companion object {
        ...
        private const val SHOW_R18_PREF_SUMMARY = "请确认您的IP不在漫画柜的屏蔽列表内，例如中国大陆IP。需要重启软件以生效。\n开启后如需关闭，需要到Sakayomi高级设置内清除Cookies后才能生效。" // "Please make sure your IP is not in Manhuagui's ban list, e.g., China mainland IP. Sakayomi restart required. If you want to close this switch after enabled it, you need to clear cookies in Sakayomi advanced setting too.
        ...
        private const val SHOW_ZH_HANT_WEBSITE_PREF_SUMMARY = "需要重启软件以生效。" // "You need to restart Sakayomi"
        ...
        private const val MAINSITE_RATELIMIT_PREF_SUMMARY = "此值影响更新书架时发起连接请求的数量。调低此值可能减小IP被屏蔽的几率，但加载速度也会变慢。需要重启软件以生效。\n当前值：%s" // "This value affects network request amount for updating library. Lower this value may reduce the chance to get IP Ban, but loading speed will be slower too. Sakayomi restart required."
        ...
        private const val IMAGE_CDN_RATELIMIT_PREF_SUMMARY = "此值影响加载图片时发起连接请求的数量。调低此值可能减小IP被屏蔽的几率，但加载速度也会变慢。需要重启软件以生效。\n当前值：%s" // "This value affects network request amount for loading image. Lower this value may reduce the chance to get IP Ban, but loading speed will be slower too. Sakayomi restart required."
