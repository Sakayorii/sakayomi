...
This guide have some instructions and tips on how to create a new Sakayomi extension. Please **read
...
free to report it through a [Meta Issue](https://github.com/Sakayorii/extensions-source/issues/new?assignees=&labels=Meta+request&template=06_request_meta.yml)
...
- Emulator or phone with developer options enabled and a recent version of Sakayomi installed
...
3. Configure remotes.
    ```bash
    # add upstream
    git remote add upstream <Sakayorii-repo-url>
...
Your extension code must be placed in the package `dev.sakayori.sakayomi.extension.<lang>.<mysourcename>`.
...
Extensions rely on [extensions-lib](https://github.com/Sakayorii/extensions-lib), which provides
some interfaces and stubs from the [app](https://github.com/Sakayorii/sakayomi) for compilation
purposes. The actual implementations can be found [here](https://github.com/Sakayorii/sakayomi/tree/main/app/src/main/java/eu/kanade/sakayomi/source).
Referencing the actual implementation will help with understanding extensions' call flow.

#### DataImage library

[`lib-dataimage`](https://github.com/Sakayorii/extensions-source/tree/main/lib/dataimage) is a library
...
[`lib-i18n`](https://github.com/Sakayorii/extensions-source/tree/main/lib/i18n) is a library for handling
...
file. Many of [the dependencies](https://github.com/Sakayorii/sakayomi/blob/main/app/build.gradle.kts)
...
> [the latest stable version of the app](https://github.com/Sakayorii/sakayomi/releases/latest).
...
depending on the `Filter` used). You can check the filter types available [here](https://github.com/Sakayorii/sakayomi/blob/main/source-api/src/commonMain/kotlin/eu/kanade/sakayomi/source/model/Filter.kt)
...
- `getMangaDetails` is called to update a manga's details from when it was initialized earlier.
    ...
    - `SManga.status` is an "enum" value. Refer to [the values in the `SManga` companion object](https://github.com/Sakayorii/extensions-lib/blob/master/library/src/main/java/eu/kanade/sakayomi/source/model/SManga.kt#L26).
...
Extensions can define a URL pattern so that these URLs can be opened in Sakayomi.
...
- `UrlActivity.kt` which should be placed next to your main file. (Example: `src/id/riztranslation/src/eu/kanade/sakayomi/extension/id/riztranslation/UrlActivity.kt`)
...
Next, you have the `<data android:scheme=“https” android:host=“host” android:pathPattern=“/..*” />` element; you can have it multiple times, which allows you to specify the URL that can be opened in Sakayomi. You can read more about this [here](https://developer.android.com/guide/topics/manifest/data-element). 
...
To explain how it works, it will trigger Sakayomi's `SEARCH` action, passing the URL as a query and specifying that it comes from your extension to narrow down the search. Avoid putting any logic in this file; instead, implement it in your extension's class.
...
```
You can find a complete example of how URLs work in the [Riztranslation extension](https://github.com/Sakayorii/extensions-source/tree/main/src/id/riztranslation).
...
To get the current `id` value before the name change, you can search the source name in the [repository JSON file](https://github.com/Sakayorii/extensions/blob/repo/index.json)
...
- `multisrc/src/main/java/eu/kanade/sakayomi/multisrc/<themepkg>/<Theme>.kt` defines the the theme's
default implementation.
- `multisrc/src/main/java/eu/kanade/sakayomi/multisrc/<theme>/<Theme>Generator.kt` defines the the
...
> [!NOTE]
> Files ending with `Gen.kt` (i.e. `multisrc/src/main/java/eu/kanade/sakayomi/multisrc/<theme>/XxxGen.kt`)
...
with open(f"{package}/src/{source}.kt", "w") as f:
    f.write(f"package dev.sakayori.sakayomi.extension.{lang}.{package}\n\n")
```
...
- `IntelijConfigurationGeneratorMainKt` should be run on creating or removing a multisrc theme.
    ...
    `/multisrc/src/main/java/eu/kanade/sakayomi/multisrc/*` before running the generator.
...
Copy the following into `Launch Flags` for the Debug build of Sakayomi:

```
-W -S -n app.sakayomi.dev/dev.sakayori.sakayomi.ui.main.MainActivity -a dev.sakayori.sakayomi.SHOW_CATALOGUES
```

For other builds, replace  `app.sakayomi.dev` with the corresponding package IDs:
- Release build: `app.sakayomi`
- Preview build: `app.sakayomi.debug`
...
Inside the `Attach Debugger to Android Process` window, once the app is running on your device and `Show all processes` is checked, you should be able to select `app.sakayomi.dev` and press OK.
...
```kotlin
package dev.sakayori.sakayomi.extension.en.mysource
