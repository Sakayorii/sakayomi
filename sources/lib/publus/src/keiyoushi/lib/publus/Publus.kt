...
class Decoder(private val data: String, private val filename: String = "configuration_pack.json") {
    ...
    private fun mod9Base64Decode(dataStr: String): State {
        ...
        while (s < 128) {
            ...
            val c1 = b64Map[dataStr[s++].code]
            val c2 = b64Map[dataStr[s++].code]
            val c3 = b64Map[dataStr[s++].code]
            val c4 = b64Map[dataStr[s++].code]
...
object PublusImage {
    fun generateFilename(pageId: String, keys: List<IntArray>, no: Int): String {
        if (keys.isEmpty() || keys[0].isEmpty()) {
            return "$pageId/$no.jpeg"
        }

        val k1 = keys[0]
        val k2 = keys[1]
        val k3 = keys[2]
        val fileName = no.toString()
    ...
    fun unscrambleNoKeys(bitmap: Bitmap, pattern: Int, blockWidth: Int = 64, blockHeight: Int = 64): Bitmap {
        ...
        val blocksX = width / blockWidth
        val blocksY = height / blockHeight
        val restX = width % blockWidth
        val restY = height % blockHeight

        var g = 0
        if (blocksX > 0) {
            g = blocksX - 43 * pattern % blocksX
            if (g % blocksX == 0) g = (blocksX - 4) % blocksX
            if (g == 0) g = blocksX - 1
        }

        var l = 0
        if (blocksY > 0) {
            l = blocksY - 47 * pattern % blocksY
            if (l % blocksY == 0) l = (blocksY - 4) % blocksY
            if (l == 0) l = blocksY - 1
        }
        ...
        if (restX > 0 && restY > 0) {
            val srcX = g * blockWidth
            val srcY = l * blockHeight
            moves.add(Move(srcX, srcY, srcX, srcY, restX, restY))
        }

        if (restY > 0) {
            for (r in 0 until blocksX) {
                val p = calcXCoordinateXRest(r, blocksX, pattern)
                val k = calcYCoordinateXRest(p, g, l, blocksY, pattern)
                val destX = calcPositionWithRest(p, g, restX, blockWidth)
                val destY = k * blockHeight
                val srcX = calcPositionWithRest(r, g, restX, blockWidth)
                val srcY = l * blockHeight
                moves.add(Move(srcX, srcY, destX, destY, blockWidth, restY))
            }
        }

        if (restX > 0) {
            for (q in 0 until blocksY) {
                val k = calcYCoordinateYRest(q, blocksY, pattern)
                val p = calcXCoordinateYRest(k, g, l, blocksX, pattern)
                val destX = p * blockWidth
                val destY = calcPositionWithRest(k, l, restY, blockHeight)
                val srcX = g * blockWidth
                val srcY = calcPositionWithRest(q, l, restY, blockHeight)
                moves.add(Move(srcX, srcY, destX, destY, restX, blockHeight))
            }
        }

        for (r in 0 until blocksX) {
            for (q in 0 until blocksY) {
                val p = (r + 29 * pattern + 31 * q) % blocksX
                val k = (q + 37 * pattern + 41 * p) % blocksY
                val t = if (p >= calcXCoordinateYRest(k, g, l, blocksX, pattern)) restX else 0
                val n = if (k >= calcYCoordinateXRest(p, g, l, blocksY, pattern)) restY else 0
                val destX = p * blockWidth + t
                val destY = k * blockHeight + n
                val srcX = r * blockWidth + (if (r >= g) restX else 0)
                val srcY = q * blockHeight + (if (q >= l) restY else 0)
                moves.add(Move(srcX, srcY, destX, destY, blockWidth, blockHeight))
            }
        }
    ...
    private fun calcPositionWithRest(a: Int, f: Int, b: Int, e: Int): Int = a * e + (if (a >= f) b else 0)

    private fun calcXCoordinateXRest(a: Int, f: Int, b: Int): Int = if (f == 0) 0 else (a + 61 * b) % f

    private fun calcYCoordinateXRest(a: Int, f: Int, b: Int, e: Int, d: Int): Int {
        val c = d % 2 == 1
        var eVal = e
        var fVal: Int
        if (if (a < f) c else !c) {
        ...
        return if (eVal == 0) 0 else (a + 53 * d + 59 * b) % eVal + fVal
    }

    private fun calcXCoordinateYRest(a: Int, f: Int, b: Int, e: Int, d: Int): Int {
        val c = d % 2 == 1
        var eVal = e
        var bVal: Int
        if (if (a < b) c else !c) {
        ...
        return if (eVal == 0) 0 else (a + 67 * d + f + 71) % eVal + bVal
    }
    ...
    private fun a3f(p1: Double, p2: Double, p3: Double, p4: Double): IntArray {
        ...
        val kArr = IntArray(maxLen)
        val lArr = IntArray(maxLen)
        ...
        val oArr = IntArray(maxLen)
        val pArr = IntArray(maxLen)
